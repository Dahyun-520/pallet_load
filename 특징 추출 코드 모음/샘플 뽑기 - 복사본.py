...
def sample_by_box_count(file_path,
    ...
    # 2️⃣ 박스 개수별 분류
    grouped = {i: [] for i in range(min_count, max_count + 1)}

    for item in all_data:
        box_length = item["input"][2]
        box_width = item["input"][3]
        box_count = item["input"][4]

        # 🔥 조건 추가: 길이가 너비보다 큰 경우만
        if box_length > box_width and min_count <= box_count <= max_count:
            grouped[box_count].append(item)

    # 3️⃣ 박스 개수별 샘플링
    final_data = []

    for box_count in range(min_count, max_count + 1):
...
output_file = "Pinwheel_sampled_big_length_40each.json"
